...
function Workspace() {
  ...
  const loadStudents = useCallback(async () => {
    ...
    if (error) notify(error.message);
    const rows = (profilesResult.data || []) as Student[];
    const activeEmails = new Set(rows.map((row) => row.email.toLowerCase()));
    setInvitations(
      ((invitationsResult.data || []) as Invitation[]).filter(
        (invite) =>
          !invite.accepted_at && !activeEmails.has(invite.email.toLowerCase()),
      ),
    );
    setStudents(rows);
...
function Overview({
  ...
  const progressFor = (id: string) => {
      ...
      return journeyProgress(record?.mentorship_start, record?.mentorship_end);
    },
...
function Students({
  ...
  async function cancelInvitation(invitation: Invitation) {
    ...
    const { error } = await app.client
      ...
      .delete()
      .eq("id", invitation.id);
    notify(error ? error.message : "Invitation cancelled");
    if (!error) reload();
  }
  return (
    <>
        ...
        {invitations.map((invite) => (
          <article key={invite.id}>
            <div className="student-person">
              <b>{initials(invite.email)}</b>
              <div>
                <strong>{invite.email.split("@")[0]}</strong>
                <small>{invite.email}</small>
              </div>
            </div>
            <span className="status pending">Invited</span>
            <button className="danger" onClick={() => cancelInvitation(invite)}>
              Cancel invitation
            </button>
          </article>
        ))}
...
function Timetable({
  ...
  async function cancelMeeting(id: string) {
    ...
    const { error } = await app.client
      ...
      .eq("id", id);
    notify(error ? error.message : "Meeting cancelled");
    loadRequests();
    reload();
  }
  ...
  return (
    <>
      ...
      {app.profile.role === "mentor" &&
        requests
          .filter((r) => ["requested", "confirmed"].includes(r.status))
          .map((r) => (
            <section className="card request-row" key={r.id}>
              <div>
                <strong>
                  {pretty(r.status)} meeting from {requester(r.mentee_id)}
                </strong>
                <p>
              ...
              <button className="soft" onClick={() => cancelMeeting(r.id)}>
                Cancel meeting
              </button>
          ...
          {rows.map((n) => (
            ...
            >
              ...
              <small>
                ...
                {n.proposal_status !== "none"
                  ? ` · Mentor edit: ${pretty(n.proposal_status)}`
                  : ""}
              </small>
            </button>
          ))}
          ...
          {noteMode === "write" && !reviewingMentorEdit ? (
            <>
              <div className="format-bar" aria-label="Note formatting">
                <button
                  title="Heading 1"
                  onClick={() => formatNote("# ", "", "Heading 1")}
                >
                  H1
                </button>
                <button
                  title="Heading 2"
                  onClick={() => formatNote("## ", "", "Heading 2")}
                >
                  H2
                </button>
                <button
                  title="Heading 3"
                  onClick={() => formatNote("### ", "", "Heading 3")}
                >
                  H3
                </button>
                <button
...
function DiffPreview({
  ...
  const output: { text: string; kind: "same" | "added" | "removed" }[] = [];
  let i = before.length,
    j = after.length;
  while (i || j) {
    if (i && j && before[i - 1] === after[j - 1]) {
      output.push({ text: before[i - 1], kind: "same" });
      i--;
      j--;
    } else if (j && (!i || matrix[i][j - 1] >= matrix[i - 1][j])) {
      output.push({ text: after[j - 1], kind: "added" });
      j--;
    } else {
      output.push({ text: before[i - 1], kind: "removed" });
      i--;
    }
  }
